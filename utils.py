...
#     w1 = np.kaiser(N, beta)
#     w = np.interp(t12, t, w1)#这里会报错，因为t和w1维度不匹配
#     w[t12 > t[-1]] = 0
#     w[t12 < t[0]] = 0
#     return w
...
def kaiser2d(N, beta):
    ...
    t1, t2 = np.meshgrid(t, t)
    t12 = np.sqrt(t1 * t1 + t2 * t2)
    w1_short = np.kaiser(N, beta)  # 长度为 N
    t_short = np.linspace(-(N - 1) / 2, (N - 1) / 2, N) / np.double(N - 1)
    w1 = np.interp(t, t_short, w1_short)  # 将 w1 插值到长度为 N+1
...
def downgrade_images(I_MS, I_PAN, ratio, sensor=None):
    ...
    if flag_resize_new == 1:
        ...
        if flag_PAN_MTF == 1:
            ...
            # bicubic resize
            I_PAN_pad = np.pad(I_PAN, int(2 * ratio), 'symmetric')
            I_PAN_LR = resize(I_PAN_pad, (I_PAN_pad.shape[0] // ratio, I_PAN_pad.shape[1] // ratio), order=3, mode='symmetric')
            #order=3表示双三次插值
            I_PAN_LR = I_PAN_LR[2:-2, 2:-2]
...
#只对LRMS图像做降采样，输入维度为(H,W,C),输出维度为(h,w,C)
def downgrade_images1(I_MS, ratio, sensor=None):
    """
    downgrade MS and PAN by a ratio factor with given sensor's gains，暂时修改为只对MS做下采样
    I_MS(H,W,C)
    """
    I_MS = np.double(I_MS)
    # I_PAN = np.double(I_PAN)
