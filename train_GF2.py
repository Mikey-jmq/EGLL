...
def PanNet(pan, lrms, epoch=100):
    ...
    (train_pan, train_lrms), \
        (val_pan, val_lrms), \
        (test_pan, test_lrms) = split_dataset(pan, lrms, size=training_size, stride=80,seed=7)
    ...
    # 数据准备
    def prepare_data(pan_grids, lrms_grids):
        """
        ...
        for i in range(N):
            ...
            lrms_grid = lrms_grid.astype(np.float32)
            pan_grid = pan_grid.astype(np.float32)
            lrms_down = cv2.resize(lrms_grid, (16, 16), interpolation=cv2.INTER_CUBIC)
            pan_down = cv2.resize(pan_grid, (64, 64), interpolation=cv2.INTER_CUBIC)
            pan_down = np.expand_dims(pan_down, -1)
            # lrms_down, pan_down = downgrade_images(lrms_grid, pan_grid, ratio)
    ...
    # 准备训练和验证数据（仅一次）
    train_pan, train_lrms, train_hrms = prepare_data(train_pan, train_lrms)
    val_pan, val_lrms, val_hrms = prepare_data(val_pan, val_lrms)
    ...
    for loss_name, loss_fn in loss_functions.items():
        ...
        checkpoint = ModelCheckpoint(f'./weightsGF2/{model_name}.h5',
        ...
        model.load_weights(f'./weightsGF2/{model_name}.h5')
        ...
        log_file = os.path.join(save_dir, f"GF2_{model_name}_log.txt")
...
def PNN(pan, lrms, epoch=100):
    ...
    (train_pan, train_lrms), \
        (val_pan, val_lrms), \
        (test_pan, test_lrms) = split_dataset(pan, lrms, size=training_size, stride=80,seed=10)
    print(f"Train set: {len(train_pan)} grids, PAN {train_pan.shape}, LRMS {train_lrms.shape}")
    print(f"Val set: {len(val_pan)} grids, PAN {val_pan.shape}, LRMS {val_lrms.shape}")
    print(f"Test set: {len(test_pan)} grids, PAN {test_pan.shape}, LRMS {test_lrms.shape}")
...
def FusionNet(pan, lrms, epoch=100):
    ...
    (train_pan, train_lrms), \
        (val_pan, val_lrms), \
        (test_pan, test_lrms) = split_dataset(pan, lrms, size=training_size, stride=80,seed=30)
    print(f"Train set: {len(train_pan)} grids, PAN {train_pan.shape}, LRMS {train_lrms.shape}")
    print(f"Val set: {len(val_pan)} grids, PAN {val_pan.shape}, LRMS {val_lrms.shape}")
    print(f"Test set: {len(test_pan)} grids, PAN {test_pan.shape}, LRMS {test_lrms.shape}")
...
def MSDCNN(pan, lrms, epoch=50):
    ...
    (train_pan, train_lrms), \
        (val_pan, val_lrms), \
        (test_pan, test_lrms) = split_dataset(pan, lrms, size=training_size, stride=80,seed=5)
    print(f"Train set: {len(train_pan)} grids, PAN {train_pan.shape}, LRMS {train_lrms.shape}")
    print(f"Val set: {len(val_pan)} grids, PAN {val_pan.shape}, LRMS {val_lrms.shape}")
    print(f"Test set: {len(test_pan)} grids, PAN {test_pan.shape}, LRMS {test_lrms.shape}")
    ...
    # 准备训练和验证数据
    train_pan, train_lrms, train_hrms = prepare_data(train_pan, train_lrms, ratio=ratio, sensor=None)
    val_pan, val_lrms, val_hrms = prepare_data(val_pan, val_lrms, ratio=ratio, sensor=None)
    print(f"Prepared data: Train PAN {train_pan.shape}, LRMS {train_lrms.shape}, HRMS {train_hrms.shape}")
    print(f"Prepared data: Val PAN {val_pan.shape}, LRMS {val_lrms.shape}, HRMS {val_hrms.shape}")
...
def GPPNN(pan, lrms,epoch=50):
    ...
    (train_pan, train_lrms), \
        (val_pan, val_lrms), \
        (test_pan, test_lrms) = split_dataset(pan, lrms, size=training_size, stride=80,seed=13)
    print(f"Train set: {len(train_pan)} grids, PAN {train_pan.shape}, LRMS {train_lrms.shape}")
    print(f"Val set: {len(val_pan)} grids, PAN {val_pan.shape}, LRMS {val_lrms.shape}")
    print(f"Test set: {len(test_pan)} grids, PAN {test_pan.shape}, LRMS {test_lrms.shape}")
...
def LAGConv(pan, lrms, epoch=50):
    ...
    (train_pan, train_lrms), \
        (val_pan, val_lrms), \
        (test_pan, test_lrms) = split_dataset(pan, lrms, size=training_size, stride=80,seed=2)
    print(f"Train set: {len(train_pan)} grids, PAN {train_pan.shape}, LRMS {train_lrms.shape}")
    print(f"Val set: {len(val_pan)} grids, PAN {val_pan.shape}, LRMS {val_lrms.shape}")
    print(f"Test set: {len(test_pan)} grids, PAN {test_pan.shape}, LRMS {test_lrms.shape}")
...
def CMFNet(pan, lrms, epoch=50):
    ...
    (train_pan, train_lrms), \
        (val_pan, val_lrms), \
        (test_pan, test_lrms) = split_dataset(pan, lrms, size=training_size, stride=80,seed=1)
    print(f"Train set: {len(train_pan)} grids, PAN {train_pan.shape}, LRMS {train_lrms.shape}")
    print(f"Val set: {len(val_pan)} grids, PAN {val_pan.shape}, LRMS {val_lrms.shape}")
    print(f"Test set: {len(test_pan)} grids, PAN {test_pan.shape}, LRMS {test_lrms.shape}")
...
###################GF2
mss_path = r"D:\桌面\对数损失\EGLL\GF2_BJ_mss.npy"
pan_path = r"D:\桌面\对数损失\EGLL\GF2_BJ_pan.npy"
used_ms = np.load(mss_path)  # 多光谱图像(1000,1000,4)
used_pan = np.load(pan_path)  # 全色图像(4000,4000)
used_pan = np.expand_dims(used_pan, -1) #(4000,4000,1)
'''normalization'''
